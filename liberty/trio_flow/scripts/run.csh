...
if ( $#argv < 1) then
    ...
    echo "Where char_type options include NOM|LVF|MC|UF|EM|AOCV|ICD"
...
#setenv ALTOSHOME /icd/flow/LIBERATE/LIBERATE211/latest/lnx86/
...
if ($CHAR_TYPE == NOM || $CHAR_TYPE == UF || $CHAR_TYPE == EM || $CHAR_TYPE == ICD) then
	set binary = liberate
else if ($CHAR_TYPE == LVF || $CHAR_TYPE == AOCV || $CHAR_TYPE == MC) then
	set binary = variety
else 
	echo "Error: Incorrect char type.  Char type should be NOM, UF, LVF, AOCV, MC or ICD"
endif
